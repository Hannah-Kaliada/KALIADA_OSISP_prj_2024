...
init ()
{
  ...
  if (current_directory_ == NULL)
    {
      printf (
          "Error Occurred. Memory allocation failed.\n");
...
scroll_up ()
{
  ...
  if (len >= maxy - 1)
    {
      // Проверяем, если текущий выбор находится в верхней
      // половине отображаемой области
      if (selection <= start + maxy / 2)
        {
          // Если начальная позиция (start) уже равна 0,
          // очищаем окно current_win
          if (start == 0)
            {
              wclear (current_win); // Очищаем окно
                                    // current_win (удаляем
              // старое содержимое)
            }
          else
            {
              start--; // Уменьшаем начальную позицию
                       // (сдвигаем список вверх)
              wclear (
                  current_win); // Очищаем окно current_win
                                // для обновления
              // содержимого
            }
        }
...
scroll_down ()
{
  ...
  selection = (selection > len - 1)
                  ? len - 1
                  : selection; // Проверяем, не превысил ли
                               // индекс выбора
  // количество файлов - 1
  ...
  if (len >= maxy - 1)
    {
      // Проверяем, если текущий выбор находится в нижней
      // половине отображаемой области
      if (selection - 1 > maxy / 2)
        {
          // Проверяем, достигла ли начальная позиция конца
          // списка
          if (start + maxy - 2 != len)
            {
              start++; // Увеличиваем начальную позицию
                       // (сдвигаем список
              // вниз)
              wclear (
                  current_win); // Очищаем окно current_win
  ...
  for (int i = 0; i < n - 1; i++)
    {
      for (int j = i + 1; j < n; j++)
        {
          // Сравниваем имена файлов и меняем их местами,
          // если необходимо
          if (strcmp (files_[i], files_[j]) > 0)
            {
              strcpy (
                  ...
                  files_[i]); // Копируем текущий элемент во
                              // временный буфер
              strcpy (files_[i],
                      files_[j]); // Заменяем текущий
                                  // элемент на следующий
              strcpy (
                  files_[j],
...
check_text (char *path)
{
  ...
  if (ptr == NULL)
    {
      perror ("Error opening file"); // Выводим сообщение об
                                     // ошибке, если не
  ...
  while ((c = fgetc (ptr)) != EOF)
    { // Читаем файл посимвольно, пока не достигнем конца
      // файла (EOF)
      if (c < 0 || c > 127)
...
read_ (char *path)
{
  ...
  ptr = fopen (
      path,
      "rb"); // Открываем файл для чтения в бинарном режиме
  if (ptr == NULL)
    {
      perror ("Error"); // Выводим сообщение об ошибке, если
                        // не удалось
      // открыть файл
      return;
    }
  ...
  do
    {
      ...
      if (is_text)
      ...
      else
        {
          fseek (ptr, pos * (maxx - 2),
                 SEEK_SET); // Перемещаем указатель в нужное
                            // место файла
          size_t bytesRead = fread (
              buffer, sizeof (unsigned char), maxx - 2,
      ...
      wrefresh (
          current_win); // Обновляем текущее окно ncurses
      ch = wgetch (current_win); // Получаем нажатую клавишу
      wclear (current_win); // Очищаем текущее окно перед
                            // обновлением

      switch (ch)
        {
        case KEY_UP:
          pos = pos == 0 ? 0 : pos - 1; // Перемещаемся
                                        // вверх по файлу
          // (уменьшаем позицию)
          break;
        case KEY_DOWN:
          pos++; // Перемещаемся вниз по файлу (увеличиваем
                 // позицию)
          break;
        }
    }
    ...
    { // Дочерний процесс
      execlp ("nano", "nano", path,
              NULL); // Запускаем программу nano для
                     // открытия файла
      perror ("exec"); // В случае ошибки выполнения exec
                       // выводим сообщение об
      // ошибке
      exit (EXIT_FAILURE); // Завершаем дочерний процесс с
                           // ошибкой
    }
  else if (pid < 0)
          ...
          if (WIFEXITED (status))
            ...
            { // Проверяем, завершился ли
              ...
              printf (
                  ...
                  WTERMSIG (status)); // Выводим информацию
                                      // о сигнале
            }
        }
    }
  ...
  while ((c = wgetch (path_win)) != '\n')
    {
      if (c == 127 || c == 8)
        ...
        {
          // Добавление введенного символа к новому имени
          // файла
          new_name[i++] = c;
          new_name[i] = '\0';
        }
...
delete_file (char *files[])
{
  ...
  // Очищаем окно path_win и выводим запрос на подтверждение
  // удаления
  wclear (path_win);
  wmove (path_win, 1, 0);
...
LOOP_:
  ...
  switch (c)
    ...
    case 'n':
    case 'N':
      break; // Ничего не делаем, пользователь отказался от
             // удаления
    default:
      goto LOOP_; // Повторяем цикл, если введенный символ
                  // некорректен
      break;
    }
}
...
handle_enter (char *files[])
{
  ...
  if (strcmp (files[selection], "..") == 0)
    ...
    {
      // Переход в выбранный каталог или открытие выбранного
      // файла
      ...
      if (S_ISDIR (file_stats.st_mode))
      ...
      else
        {
          ...
          // Добавляем "/" к текущему каталогу, если это
          // необходимо
      ...
      if (strcmp (dir_entry->d_name, ".") != 0
          && strcmp (dir_entry->d_name, "..") != 0)
        {
          ...
          if (lstat (temp_path, &file_stat) == -1)
            {
              continue; // Пропускаем элемент в случае
                        // ошибки
            }

          // Если текущий элемент является каталогом,
          // рекурсивно вычисляем его размер
          ...
          else
            {
              ...
              directory_size
                  += (float)(file_stat.st_size)
                     / (float)1024; // Преобразуем размер в
                                    // KB
            }
        }
    }
...
show_file_info (char *files[])
{
  ...
  // Проверяем, что выбранный элемент не является ссылкой на
  ...
  if (strcmp (files[selection], "..") != 0)
    {
      ...
      // Получаем информацию о выбранном элементе с помощью
      // lstat
      if (lstat (temp_address, &file_stats) == 0)
        {
          ...
          wprintw (info_win, (file_stats.st_mode & S_IRUSR)
                                 ? "r"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IWUSR)
                                 ? "w"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IXUSR)
                                 ? "x"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IRGRP)
                                 ? "r"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IWGRP)
                                 ? "w"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IXGRP)
                                 ? "x"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IROTH)
                                 ? "r"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IWOTH)
                                 ? "w"
                                 : "-");
          wprintw (info_win, (file_stats.st_mode & S_IXOTH)
                                 ? "x"
                                 : "-");
          wprintw (info_win, "\n");
        ...
        {
          // Если возникла ошибка при получении информации о
          // файле
          perror ("Error getting file status");
        }
    }
...
XOR ()
{
  for (int j = 1; j < N; j++)
    {
      checked_value[j]
          = ((checked_value[j] == gen_poly[j]) ? '0' : '1');
    }
}
...
{
  // Используем функцию qsort для сортировки массива строк с
  // помощью кастомного компаратора
  ...
  while ((c = wgetch (info_win)) != '\n')
    {
      ...
      wmove (info_win, 1, 1);
      wprintw (info_win,
               "Enter the regex for searching: %s", regexp);
      wrefresh (info_win);
    }
  int num_matches;
  // Ищем файлы с использованием регулярного выражения
  char **file_paths = find_files_by_regex (root_dir, regexp,
                                           &num_matches);
  ...
  if (ch == KEY_ENTER || ch == '\n')
    {
      ...
      if (parent_dir == NULL)
        {
          perror ("Error allocating memory for parent "
                  "directory path");
          free (file_path_copy);
          return;
        }
      // Копируем путь к родительскому каталогу в parent_dir
      ...
      // Освобождаем скопированный путь к файлу и выделенную
      // память для parent_dir
      free (file_path_copy);
      free (parent_dir);
    }
  ...
  while ((entry = readdir (dir)) != NULL)
    {
      ...
      // Получаем информацию о файле (с учетом символических
      // ссылок)
      struct stat statbuf;
      ...
      else if (S_ISREG (statbuf.st_mode))
        {
          ...
          if (regexec (regex, entry->d_name, 0, NULL, 0)
              == 0)
            {
              ...
              // Проверяем, не превысили ли максимальное
